...
def sidebar():
    with st.sidebar:
        ...
        arxiv_top_k = st.slider("Arxiv Search Top K", 1, 5, 2)
        rerank_k = st.slider("Rerank K", 1, 5, 3)
        rerank_threshold = st.slider("Rerank Threshold", 0.0, 1.0, 0.01)
        search_type = st.pills("Search Type", options=["hybrid_search", "hybrid_search2"], default=["hybrid_search"], selection_mode="single")
        return doc_top_k, web_top_k, arxiv_top_k, rerank_k, rerank_threshold, search_type
...
def main(doc_top_k:int, web_top_k:int, arxiv_top_k:int, rerank_k:int, rerank_threshold:float, search_type:str):
    # User input
    if prompt := st.chat_input("Ask me anything"):
        ...
        search_payload = {
            ...
            "top_k": {"doc": doc_top_k, "web": web_top_k, "arxiv": arxiv_top_k},
            "rerank_k": rerank_k,
...
if __name__ == "__main__":
    ...
    doc_top_k, web_top_k, arxiv_top_k, rerank_k, rerank_threshold, search_type = sidebar()
    chat_history()
    main(doc_top_k, web_top_k, arxiv_top_k, rerank_k, rerank_threshold, search_type)
    debug()